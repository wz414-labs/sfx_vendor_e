...
EXPORTS_VALS="EOS_TMP_DIR EOS_ZIP_DIR EOS_USE_CCACHE EOS_CCACHE_DIR EOS_CCACHE_SIZE EOS_BRANCH_NAME EOS_REPO EOS_MIRROR EOS_OTA_URL EOS_INCLUDE_PROPRIETARY EOS_BUILD_OVERLAY EOS_LOCAL_MIRROR EOS_CLEAN_ZIPDIR EOS_CRONTAB_TIME EOS_CLEAN_AFTER_BUILD EOS_CLEAN_BEFORE_BUILD EOS_WITH_SU EOS_ANDROID_JACK_VM_ARGS EOS_CUSTOM_PACKAGES EOS_SIGN_BUILDS EOS_KEYS_SUBJECT EOS_ZIP_SUBDIR EOS_LOGS_SUBDIR EOS_SIGNATURE_SPOOFING EOS_DELETE_OLD_ZIPS EOS_DELETE_OLD_LOGS EOS_SYNC_RESET EOS_MINI_APPS EOS_BUILD_USER EOS_CCACHE_COMPRESS EOS_CCACHE_COMPRESSLEVEL EOS_CCACHE_EXEC"
...
# 1. they have big server farms where ccache makes no difference anymore
# 2. when building for a bunch of different devices it caused issues
# 3. the old prebuilt ccache they used caused issues
#
# so the other way around that means:
# 1. if you do not have a big server farm: use ccache
# 2. if you build for different devices use separate ccache paths!
# 3. as you have to use the system ccachesince Q we hopefully have/get all issues fixed from there
# source: https://cs.android.com/android/platform/superproject/+/android10-release:build/make/core/ccache.mk;l=17-24
#
# define EOS_CCACHE_EXEC in your device/<vendor>/<codename>/vendorsetup.sh.
CCACHE_EXEC="$EOS_CCACHE_EXEC"
# if not defined in the device vendorsetup.sh the following will be used instead:
